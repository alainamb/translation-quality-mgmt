    ...
    boundaries.forEach(boundary => {
        ...
        if (boundary.pos > lastPos) {
            ...
            if (activeAnnotations.length > 0) {
                ...
                const severity = getSeverity(primaryAnnotation.annotation);
                const cssClasses = `highlight ${category.replace(/\s+/g, '-')} ${severity}`;
...
function displayLegend() {
    ...
    // Add severity legend as a separate section
    legendHtml += '<div class="legend-section">';
    legendHtml += '<h4>Severity Levels</h4>';
    legendHtml += '<div class="legend-items">';
    legendHtml += `
        <div class="legend-item">
            <div class="legend-color severity-major-legend"></div>
            <span>Major</span>
        </div>
        <div class="legend-item">
            <div class="legend-color severity-minor-legend"></div>
            <span>Minor</span>
        </div>
    `;
    legendHtml += '</div></div>';
...
function displaySummaryTable() {
    ...
    textAnnotations.forEach(annotation => {
        ...
        const severityBadge = `<span class="error-badge severity-${severity}">${severity}</span>`;
...
function showTooltip(event, markerMap) {
    ...
    let tooltipContent = `
        <div class="tooltip-info"><strong>Category:</strong> ${category.charAt(0).toUpperCase() + category.slice(1)}</div>
        <div class="tooltip-info"><strong>Type:</strong> ${escapeHtml(subcategory)}</div>
        <div class="tooltip-info"><strong>Severity:</strong> ${severity}</div>
    `;
...
function getSeverity(annotation) {
    const result = evaluationData.annotation_data.result;
    const severityAnnotation = result.find(ann => 
        ann.from_name === 'severity' && 
        ann.id === annotation.id
    );
    
    if (severityAnnotation && severityAnnotation.value && severityAnnotation.value.choices) {
        return severityAnnotation.value.choices[0].toLowerCase();
    }
    return 'minor';
}
